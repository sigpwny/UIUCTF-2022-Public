...
class K8sClient(object):
    ...
    def from_config(cls, config, *, namespace=None, **kwargs):
        ...
        context = self._find_cfg_obj(
            "contexts", config["current-context"]
        )
        cluster = self._find_cfg_obj("clusters", context["cluster"])
        self.server = cluster["server"]
        self.namespace = namespace or context["namespace"]

        user = self._find_cfg_obj("users", context["user"])
